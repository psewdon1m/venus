...
export function Dashboard() {
      ...
      if (editingPersona) {
        ...
        setPersonas(personas.map(p =>
          p.id === editingPersona.id
            ? { ...p, displayName: personaForm.displayName, manifest: personaForm.manifest }
            : p
        ));
      } else {
        // Create new persona
  ...
  const handlePublishPersona = async (personaId: string) => {
    try {
      const response = await apiClient.publishPersona(personaId);
      // Update persona in local state
      setPersonas(personas.map(p =>
        p.id === personaId
          ? { ...p, isPublic: true, publicSlug: response.data.persona.publicSlug }
          : p
      ));
  ...
  const openAssignmentModal = async (persona: Persona) => {
                    ...
                    <div key={persona.id} className="bg-white overflow-hidden shadow rounded-lg">
                      ...
                      <div className="bg-gray-50 px-5 py-3">
                        <div className="text-sm space-x-3">
                          ...
                          <button
                            onClick={() => openAssignmentModal(persona)}
                            className="text-green-600 hover:text-green-500 font-medium"
                          >
                            Manage Projects
                          </button>
                          <button
                            onClick={() => handlePublishPersona(persona.id)}
              ...
              <form onSubmit={handlePersonaSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setPersonaForm(prev => ({ ...prev, manifest: e.target.value }))}
                    rows={3}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                ...
                {!editingPersona && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setPersonaForm(prev => ({ ...prev, slug: e.target.value }))}
                      className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {personaProjects.length === 0 ? (
                        ...
                        personaProjects.map((project) => (
                          <div key={project.id} className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-md">
                            <div>
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {availableProjects.length === 0 ? (
                        ...
                        availableProjects.map((project) => (
                          <div key={project.id} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-md">
                            <div>
                              <p className="text-sm font-medium text-gray-900">{project.title}</p>
                              <p className="text-xs text-gray-500">{project.type} • {project.status}</p>
                            </div>
